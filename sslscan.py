        ...
        if 'errors' not in response.json():
            while response.json()['status'] != 'READY':
                if response.json()['status'] == "ERROR":
                    print("\nL'host " + self.nome + " non e' stato scansionato ")
                    break
                else:
                    time.sleep(20)
                    response = requests.get('https://api.ssllabs.com/api/v3/analyze', params=params)
        else:
            print("\nHost + " + self.nome + " non scansionabile ")
            i=0
            for i in response.json()['errors']:
                lista_det.append(response.json()['errors'][i]['message'])
                lista_err.append(self.nome)
        ...
        while err == 0:
            ...
            # SCANSIONE FILE HOST
                ...
                if len(lista_err) > 0:
                    ...
                    worksheet_err.write('A1', 'Errore')
                    n = 2
                    m = 0
                    for l in lista_err:
                        worksheet_err.write('A' + str(n), l)
                        worksheet_err.write('A' + str(n), lista_det[m])
                        n = n + 1
                        m = m + 1
                    workbook_err.close()
